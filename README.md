...
```
RNDISDriver/
├── RNDISDriver.xcodeproj/        # Xcode 工程文件（手写 pbxproj）
...
### Xcode / DriverKit 版本兼容性

| Xcode 版本 | DriverKit SDK | macOS 最低版本 | 状态 |
|-----------|---------------|---------------|------|
| **Xcode 26.5** | DriverKit 25.5 / 26.x | macOS 26.0 | 主要开发环境，CI 构建通过 |
| Xcode 16.x | DriverKit 23.x（随 macOS SDK） | macOS 14.0 | 需条件编译（`registerEthernetInterface` 参数差异），见下方兼容性说明 |
| Xcode 15.x | 无独立 DriverKit SDK | macOS 13.0 | 不建议，NetworkDriverKit API 变动大 |

> **兼容性说明**：项目使用 `#if __has_include(<NetworkingDriverKit/IOUserNetworkRxSubmissionQueue.h>)` 做编译时 SDK 检测，在旧版 DriverKit SDK（23.x）下自动切换到 4 参数版 `registerEthernetInterface` 调用。Network 类名也随 SDK 版本做条件适配（`IOUserNetworkEthernet` vs `IOUserEthernetController`）。

### 环境
- macOS 13.0+ (Ventura)
- Xcode 26.5 推荐，Xcode 16.x 可编译
...
| Job | Runner | 说明 |
|-----|--------|------|
| `build` | `macos-26` (Apple Silicon) | Debug + Release 双配置无签名构建，Xcode 26.5 + DriverKit SDK |
| `lint` | `macos-26` | 校验所有 `.plist` 和 `.entitlements` 格式合法性 |
| `release` | `macos-26` | 仅 `workflow_dispatch` 手动触发，签名 + 公证（需配置 Secrets） |

**CI 前置条件**：
- `.xcodeproj` 已提交到仓库（项目自带手写 pbxproj）
...
|----------|----------------------|
| `IOEthernetController` | `IOUserNetworkEthernet`（Xcode 26+）/ `IOUserEthernetController`（Xcode 16-） |
| `IOEthernetInterface` | 框架内部管理 |
...
| `IOGatedOutputQueue` | `IOUserNetworkPacketQueue` / 框架内置队列 |
| `IOLog` | `os_log` |
| `IOSleep` | `IOSleep` (可用) |
| RNDIS 协议逻辑 | **完全可复用** |
